...
import { fixture } from "./interactions/flight.mjs";
...
const result = await page.evaluate(() => ({
  ...
  cspAllowsProvider: ["https://api.aviationstack.com","https://api.airplanes.live"].every(h => document.querySelector('meta[http-equiv="Content-Security-Policy"]')?.content.includes(h)),
...
if (result.title !== "AA100" || result.route.join("-") !== "JFK-LAX" || !result.mapVisible ||
    result.keyExposed || !result.cspAllowsProvider || !/Airplanes\.live ADS-B/.test(result.positionSource) || !result.cached || issues.length) {
