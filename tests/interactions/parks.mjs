...
export async function interact({ page, log, evidenceDir }) {
  ...
  /* ---- 5. invalid-key path: API answers 403 -> the "That key didn't work" card ----
     Runs on a sibling page in the same context: an intentionally induced HTTP 403 makes
     the browser emit console.error "Failed to load resource ... 403", which the harness
     counts as a hard issue against the main interaction page (only net::ERR is filtered).
     The sibling page gives identical end-to-end coverage (context routes apply to it),
     with the evidence logged here + screenshot invalid-key-state.png. */
  ...
  await p2.evaluate(k => {
    localStorage.setItem("suite.key.nps", k);          // self-seed in case file:// storage isn't shared
    localStorage.removeItem("suite.cache.parks.parklist"); // force a parks refetch on boot
  }, FAKE_KEY);
  await p2.reload();
  await p2.waitForSelector('#app .card-msg h3:has-text("That key didn\'t work")', { timeout: 15000 });
  log(`403 from API -> invalid-key card: "${(await p2.textContent("#app .card-msg h3")).trim()}", ` +
    `input cleared: ${(await p2.inputValue('#app input[aria-label="NPS API key"]')) === ""}`);
  await p2.screenshot({ path: evidenceDir + "/invalid-key-state.png", fullPage: true });
  await p2.close();
  await page.context().unroute("https://developer.nps.gov/**");
