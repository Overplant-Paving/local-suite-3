/* tests/interactions/transit.mjs — Transit Departure Board (Batch C, remediation)
   ...
   Etiquette note: total real api.bart.gov requests in the v2 pass: 1 stations +
   3 ETD (initial, station change, recovery). BART's ETD API is built for polling;
   no loops, no retries. */
...
export async function interact({ page, log, evidenceDir }) {
  ...
  let relayHit = null;
  await page.route(u => u.href.startsWith(RELAY_BASE + "?url="), r => {
  ...
  await page.unroute(u => u.href.startsWith(RELAY_BASE + "?url="));
